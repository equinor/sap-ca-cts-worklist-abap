...
* only works with lines up to 255 characters
    ...
    METHODS get
      RETURNING
        VALUE(rt_diff) TYPE zif_cts_definitions=>ty_diffs_tt .
    METHODS stats
      RETURNING
        VALUE(rs_count) TYPE zif_cts_definitions=>ty_count .
    ...
    METHODS get_beacons
      RETURNING
        VALUE(rt_beacons) TYPE zif_cts_definitions=>ty_string_tt .
  PROTECTED SECTION.

  PRIVATE SECTION.

    DATA mt_beacons TYPE zif_cts_definitions=>ty_string_tt .
    DATA mt_diff TYPE zif_cts_definitions=>ty_diffs_tt .
    DATA ms_stats TYPE zif_cts_definitions=>ty_count .
...
CLASS zcl_cts_diff IMPLEMENTATION.
    ...
    DO.
      ...
      IF sy-subrc = 0.
        ...
        CASE ls_delta-vrsflag.
          WHEN zif_cts_definitions=>c_diff-delete.
            _append '' zif_cts_definitions=>c_diff-delete ls_delta-line.
            lv_oindex = lv_oindex + 1.
          WHEN zif_cts_definitions=>c_diff-insert.
            _append ls_delta-line zif_cts_definitions=>c_diff-insert ''.
            lv_nindex = lv_nindex + 1.
          WHEN zif_cts_definitions=>c_diff-update.
            CLEAR ls_new.
            READ TABLE it_new INTO ls_new INDEX lv_nindex.
            ASSERT sy-subrc = 0.
            _append ls_new zif_cts_definitions=>c_diff-update ls_delta-line.
            lv_nindex = lv_nindex + 1.
            lv_oindex = lv_oindex + 1.
          WHEN OTHERS.
            ASSERT 0 = 1.
  ...
  METHOD set_patch_new.
    ...
    IF sy-subrc <> 0.
      zcx_cts_exception=>raise( |Invalid new line number { iv_line_new }| ).
    ENDIF.
  ...
  METHOD set_patch_old.
    ...
    IF sy-subrc <> 0.
      zcx_cts_exception=>raise( |Invalid old line number { iv_line_old }| ).
    ENDIF.
  ...
  METHOD unpack.
    ...
    lv_new = zcl_cts_convert=>xstring_to_string_utf8( iv_new ).
    lv_old = zcl_cts_convert=>xstring_to_string_utf8( iv_old ).

    SPLIT lv_new AT zif_cts_definitions=>c_newline INTO TABLE et_new.
    SPLIT lv_old AT zif_cts_definitions=>c_newline INTO TABLE et_old.
