...
CLASS zcx_cts_exception DEFINITION
  ...
  PUBLIC SECTION.
    ...
    "! @raising zcx_cts_exception | Exception
    CLASS-METHODS raise
      IMPORTING
        !iv_text     TYPE clike
  ...
  PRIVATE SECTION.
    CONSTANTS:
      gc_generic_error_msg TYPE string VALUE `An error occured (zcx_cts_exception)` ##NO_TEXT.
...
CLASS zcx_cts_exception IMPLEMENTATION.
  ...
  METHOD get_longtext.
    ...
    IF if_t100_message~t100key IS NOT INITIAL.
      ...
      CREATE OBJECT lo_message_helper TYPE ('ZCL_CTS_MESSAGE_HELPER')
        EXPORTING
          ii_t100_message = me.
  ...
  METHOD raise.
    ...
    RAISE EXCEPTION TYPE zcx_cts_exception
      EXPORTING
        textid   = ls_t100_key
        msgv1    = lv_msgv1
        msgv2    = lv_msgv2
